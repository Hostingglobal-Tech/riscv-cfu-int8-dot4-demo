...
This public fork adds a focused CFU Playground proof of concept:

- RISC-V firmware runs a deterministic signed `int8` dot product.
- A CPU-only C implementation is compared with a CFU custom-instruction implementation.
- The CFU path calls `cfu_op0()` to reset, accumulate four signed `int8` multiply-adds per instruction, and read the accumulator.
- Renode/Robot verifies the UART output and confirms `VERIFY = OK`.

Current verified Renode output:
...
- Project: [`proj/pro_nms_cfu_demo`](proj/pro_nms_cfu_demo)
- CFU hardware logic: [`proj/pro_nms_cfu_demo/cfu.py`](proj/pro_nms_cfu_demo/cfu.py)
- C firmware demo: [`proj/pro_nms_cfu_demo/src/pro_cfu_demo.c`](proj/pro_nms_cfu_demo/src/pro_cfu_demo.c)
- Menu entry: [`proj/pro_nms_cfu_demo/src/proj_menu.cc`](proj/pro_nms_cfu_demo/src/proj_menu.cc)
- Renode/Robot test: [`proj/pro_nms_cfu_demo/pro_nms_cfu_demo.robot`](proj/pro_nms_cfu_demo/pro_nms_cfu_demo.robot)
- Captured output: [`proj/pro_nms_cfu_demo/docs/renode-output.txt`](proj/pro_nms_cfu_demo/docs/renode-output.txt)
...
The WSL/Renode run is a simulation proof of the RISC-V custom-instruction path, not a physical FPGA speed measurement. Actual hardware acceleration must be measured on an FPGA board or equivalent hardware target.

This work is based on Google CFU Playground. The original project remains under Apache-2.0; see [LICENSE](LICENSE).
